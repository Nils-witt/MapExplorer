...
function OverlaySettingsComponent({ styleUrl }: OverlaySettingsComponentProps) {
  const {
    overlays,
  ...
  return (
    <Stack spacing={1.5}>
      ...
      {overlays.length === 0 ? (
        ...
        <>
          <Typography variant="body2" color="text.secondary">
            Order controls stacking on the map — the top overlay below is drawn
            on top.
          </Typography>
              ...
              editingOverlayId === overlay.id ? (
                                ...
                                <InputAdornment position="end">
                                  <Tooltip title="Use server token">
                                    <IconButton
                                      ...
                                      onClick={(event) =>
                                        handleUseServerToken(event, 'edit')
                                      }
                                    >
                ...
                <ListItem
                  ...
                  secondaryAction={
                    <Stack direction="row" spacing={0.5}>
                      <OverlayCacheButton overlay={overlay} servers={servers} />
                      <IconButton
                        edge="end"
                ...
                >
                  ...
                  >
                    ...
                    >
                      {Math.round(
                        (overlay.opacity ?? DEFAULT_OVERLAY_OPACITY) * 100,
                      )}
                      %
                    </Typography>
                  </Stack>
      ...
      <Stack component="form" spacing={1} onSubmit={handleAddOverlaySubmit}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          <TextField
            label="Name"
            size="small"
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          <TextField
            ...
            onChange={(event) => setNewOverlayAuthHeader(event.target.value)}
            placeholder="Bearer <token>"
            slotProps={{
              input: {
                endAdornment:
